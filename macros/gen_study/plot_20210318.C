...
void plot_20210318(const Bool_t debug=false) {
  ...
  TFile *const fileTTNumCorrectElectron                   =  funOpenFile(nameDatagroupTT,          nameClusterIDTT,          "NumCorrect" + namesLepton[0]);
  ...
  if (true) {
    ...
    << "|-" << "|-" << "|\n"
    << "| " << "At least 2 loose electrons in each event" << " | " << funRatioFromBoolHist(static_cast<TH1*>(fileSignalHeavyGenElectron->Get("hhave2MoreElectron_mvaFall17V2Iso_WPL"))) << " |\n"
    << "| " << "At least 2 electrons in each event" << " | " << funRatioFromBoolHist(static_cast<TH1*>(fileSignalHeavyGenElectron->Get("hhave2MoreElectron"))) << " |\n"
