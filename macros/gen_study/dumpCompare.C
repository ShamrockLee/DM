...
#ifndef DUMP_COMPARE_H
#define DUMP_COMPARE_H
void dumpCompare(
    ...
    Int_t indexOrder = 1);
void dumpCompare(
    TString pathFileIn1, TString pathFileIn2, Bool_t normalize, TString endfix1,
    TString endfix2,
    std::function<
        std::function<void(TCanvas *c, TString nameHist, UInt_t iPage)>(
            TString endfix1, TString endfix2)>
        funFunPrint,
    std::function<std::function<void(TCanvas *c, UInt_t nPage)>(
        TString endfix1, TString endfix2)>
        funFunEndPrint,
    Int_t indexOrder = 1);
void dumpCompare(TString pathFileIn1, TString pathFileIn2, Bool_t normalize,
                 TString endfix1, TString endfix2,
                 std::function<void(TCanvas *, TString, UInt_t)> funPrint,
                 std::function<void(TCanvas *, UInt_t)> funEndPrint,
                 Int_t indexOrder = 1) {
  return dumpCompare(
      pathFileIn1, pathFileIn2, normalize, endfix1, endfix2,
      [funPrint](TString endfix1, TString endfix2) { return funPrint; },
      [funEndPrint](TString endfix1, TString endfix2) { return funEndPrint; },
      indexOrder);
}
...
void dumpCompare(
  ...
  std::function<void(TCanvas * c, TString nameHist, UInt_t iHist)> funPrint =
      funFunPrint(arrEndfix[0], arrEndfix[1]);
  std::function<void(TCanvas * c, UInt_t nHist)> funEndPrint =
      funFunEndPrint(arrEndfix[0], arrEndfix[1]);
  dumpCompare(arrTFileIn[0], arrTFileIn[1], normalize, arrEndfix[0],
              arrEndfix[1], funPrint, funEndPrint, indexOrder);
  for (UInt_t iTDir = [; iTDir < nTDir; iTDir++]) {
    arrTFileIn[iTDir]->Close();
    delete arrTFileIn[iTDir];
  }
}

void dumpCompareMerged(TDirectory *tdir1, TDirectory *tdir2, Bool_t normalize,
                       TString endfix1, TString endfix2,
                       TString pathDir, TString nameFile,
                       TString optionPrint, Int_t indexOrder) {
...
void dumpCompareMerged(TString pathFileIn1, TString pathFileIn2,
                       Bool_t normalize, TString endfix1,
                       TString endfix2, TString nameDir,
                       TString nameFile, TString optionPrint,
                       Int_t indexOrder) {
  const UInt_t nTDir = 2;
  TString arrPathFileIn[nTDir] = {pathFileIn1, pathFileIn2};
  TString arrEndfix[nTDir] = {endfix1, endfix2};
...
void dumpCompareSeperated(
    TDirectory *tdir1, TDirectory *tdir2, Bool_t normalize, TString endfix1,
    TString endfix2, TString pathDir = "",
