...
void plot_20210304(const Bool_t debug=false) {
  ...
  const TString nameFolderToday = "output_" + nameCondorPack + "_20210304";
  ...
  const TString nameClusterIDSignalHeavy = "20210304";
    ...
    std::vector<TString> vName = {};
      ...
      for (const auto& nameHist: vName) {
        ...
        if (nameLeaf.BeginsWith("ElectronPair_")) {
          ...
          if (gPad) gPad->SetLogy(false);
          drawHistsMultiple(vIsHist, vHist, vNameLeg, nameLeaf, nameLeaf, vHist[0]->GetXaxis()->GetTitle());
          gPad->SetTitle(nameLeaf);
          gPad->Print(pathOutImages + "/" + nameFolderToday + "/" + "Preselections_" + nameLeaf + "." + suffixImage);
          gPad->SetLogy(true);
          gPad->Print(pathOutImages + "/" + nameFolderToday + "/" + "Preselections_" + nameLeaf + "_logy." + suffixImage);
  ...
  if (true) {
    ...
    << "| " << "At least two d-AK4 match" << " | " << funRatioFromBoolHist(static_cast<TH1*>(arrFileSignalHeavyPreselectedElectron[0]->Get("hhave2JetPassed"))) << " |\n"
    << "| " << "At least two ddbar-AK8 match" << " | " << funRatioFromBoolHist(static_cast<TH1*>(arrFileSignalHeavyPreselectedElectron[1]->Get("hhave2FatJetPassed"))) << " |\n"
