...
void plot_20210220(const Bool_t debug=false) {
  ...
  const TString nameFolderToday = "output_" + nameCondorPack + "_20210225";
  ...
  const TString nameClusterIDSignalHeavy = "20210202";
      ...
      for (const auto& nameHist: vName) {
        ...
        if (nameLeaf.BeginsWith("Electron_")) {
          ...
          std::vector<TString> vNameLeg = {};
          {
            const TString nameDatagroupShorter = "SignalHeavy";
            {
              ...
              vNameLeg.push_back(Form("%s_Gen_%s", nameDatagroupShorter.Data(), nameLeafToGet.Data()));
              vHist.push_back(static_cast<TH1*>(fileSignalHeavyNumCorrectElectron->Get("h" + nameLeafToGet)));
              vIsHist.push_back(vHist.back() != nullptr && !vHist.back()->IsZombie());
              vNameLeg.push_back(Form("%s_NumCorrect_%s", nameDatagroupShorter.Data(), nameLeafToGet.Data()));
              vHist.push_back(static_cast<TH1*>(fileSignalHeavyZMassCuttedElectron->Get("h" + nameLeafToGet)));
              vIsHist.push_back(vHist.back() != nullptr && !vHist.back()->IsZombie());
              vNameLeg.push_back(Form("%s_ZMassCutted_%s", nameDatagroupShorter.Data(), nameLeafToGet.Data()));
            }
            {
              const TString nameLeafToGet = namesLepton[0] + "NumCorrect" + tstrUnderscoreAndSuffix;
              vHist.push_back(static_cast<TH1*>(fileSignalHeavyNumCorrectElectron->Get("h" + nameLeafToGet)));
              vIsHist.push_back(vHist.back() != nullptr && !vHist.back()->IsZombie());
              vNameLeg.push_back(Form("%s_NumCorrect_%s", nameDatagroupShorter.Data(), nameLeafToGet.Data()));
              vHist.push_back(static_cast<TH1*>(fileSignalHeavyZMassCuttedElectron->Get("h" + nameLeafToGet)));
          ...
          }
          {
            ...
            {
              const TString nameLeafToGet = nameLeaf;
              vHist.push_back(static_cast<TH1*>(fileDYJetsZMassCuttedElectron->Get("h" + nameLeafToGet)));
              vIsHist.push_back(vHist.back() != nullptr && !vHist.back()->IsZombie());
              vNameLeg.push_back(Form("%s_NumCorrect_%s", nameDatagroupShorter.Data(), nameLeafToGet.Data()));
              vHist.push_back(static_cast<TH1*>(fileDYJetsZMassCuttedElectron->Get("h" + nameLeafToGet)));
              vIsHist.push_back(vHist.back() != nullptr && !vHist.back()->IsZombie());
          ...
          gPad->Print(pathOutImages + "/" + nameFolderToday + "/" + "Preselections_" + nameLeaf + "." + suffixImage);
          gPad->SetLogy(true);
          gPad->Print(pathOutImages + "/" + nameFolderToday + "/" + "Preselections_" + nameLeaf + "_logy." + suffixImage);
        }
        if (nameLeaf.BeginsWith("ElectronPair_")) {
          ...
          }
          {
            ...
            {
              const TString nameLeafToGet = nameLeaf;
              vHist.push_back(static_cast<TH1*>(fileDYJetsNumCorrectElectron->Get("h" + nameLeafToGet)));
              vIsHist.push_back(vHist.back() != nullptr && !vHist.back()->IsZombie());
              vNameLeg.push_back(Form("%s_NumCorrect_%s", nameDatagroupShorter.Data(), nameLeafToGet.Data()));
              vHist.push_back(static_cast<TH1*>(fileDYJetsZMassCuttedElectron->Get("h" + nameLeafToGet)));
              vIsHist.push_back(vHist.back() != nullptr && !vHist.back()->IsZombie());
  ...
  if (true) {
    ...
    << "|-" << "|-" << "|\n"
    << "| " << "Exactly 2 loose e" << " | " << funRatioFromBoolHist(static_cast<TH1*>(fileSignalHeavyGenElectron->Get("his" + namesLepton[0] + "NumCorrect"))) << " |\n"
    ...
    << "| " << " | " << " |\n"
    << "| " << "after preselections:" << " |\n"
    << "| " << "At least one d-AK4 match" << " | " << funRatioFromBoolHist(static_cast<TH1*>(arrFileSignalHeavyPreselectedElectron[0]->Get("hhasJetPassed"))) << " |\n"
    << "| " << "At least one ddbar-AK8 match" << " | " << funRatioFromBoolHist(static_cast<TH1*>(arrFileSignalHeavyPreselectedElectron[1]->Get("hhasFatJetPassed"))) << " |\n"
